...
namespace BlueGOAP
{
    public abstract class GoalManagerBase<TAction, TGoal> : IGoalManager<TGoal>
    {
        ...
        {
            goalsDic.Remove(goalLabel);
        }

        public IGoal<TGoal> GetGoal(TGoal goalLabel)
        {
            if (goalsDic.ContainsKey(goalLabel))
            {
                return goalsDic[goalLabel];
            }
        ...
        {
            foreach (KeyValuePair<TGoal, IGoal<TGoal>> goal in goalsDic)
            {
